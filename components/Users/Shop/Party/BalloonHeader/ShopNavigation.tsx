...
const GCT_ID = "a72e830e-69ba-4977-b8e9-9250a196dd50";
...
export const ShopNavigation: React.FC = () => {
  ...
  const fetchTabs = async () => {
    try {
      ...
      const reversed = categoryList.reverse(); // descending
    ...
    const initializeTabs = async () => {
      ...
      if (PCT && loaded.length > 0) {
        ...
        if (foundIndex !== -1) {
          ...
          setVisibleStartIndex(Math.max(0, foundIndex - Math.floor(MAX_VISIBLE_TABS / 2)));
        }
      }
    };
  ...
  return (
    <nav
      ...
      role="tablist"
      aria-label="Theme Party categories"
    >
      {/* Desktop View */}
