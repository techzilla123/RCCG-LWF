...
const GCT_ID = "a48bac0e-05b1-4511-a23e-99e31dc6abec";
...
export const ShopNavigation: React.FC = () => {
  ...
  React.useEffect(() => {
    const fetchAndInitTabs = async () => {
      const fetchedCategories = await fetchTabs();

      const PCT = searchParams.get("PCT");
      if (PCT && fetchedCategories.length > 0) {
        ...
        if (foundIndex !== -1) {
          ...
          const newStart = Math.max(
            0,
            foundIndex - Math.floor(MAX_VISIBLE_TABS / 2)
          );
  ...
  const handleTabClick = (index: number) => {
    ...
    if (selectedCategory?.categoryId) {
      router.push(`?PCT=${selectedCategory.categoryId}`);
    }
  };
