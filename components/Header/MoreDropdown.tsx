...
export const MoreDropdown: React.FC = () => {
  return (
    <>
      {/* ================= DESKTOP ================= */}
      <div className="hidden md:block absolute top-full right-0 mt-2 bg-white shadow-lg border border-gray-200 rounded-md z-50 w-[700px]">
        <div className="grid grid-cols-2">
          {/* Left Column - Dark Background */}
          <div className="bg-gray-800 text-white p-6 rounded-l-md">
            <h3 className="text-sm font-bold uppercase mb-4 border-b border-gray-600 pb-2">
              TAKE YOUR NEXT STEP
            </h3>
...
<div className="md:hidden w-full">
  <div className="bg-gray-800 text-white rounded-lg p-4">
    <h3 className="text-sm font-bold uppercase mb-3">Take Your Next Step</h3>
    <div className="space-y-4">
      <MobileItem href="/baptism" label="Baptism" />
      <MobileItem href="/ministries" label="Ministries" />
      <MobileItem href="/class" label="Next Believer's Class" />
  ...
  <div className="bg-white mt-4 rounded-lg p-4">
    <h4 className="text-xs font-bold uppercase mb-3 text-zinc-800">Ministries</h4>
    <MobileMinistry href="/ministries#children" title="Children’s Ministry" desc="Ages 1 – 12" />
    <MobileMinistry href="/ministries#youth" title="Youth Ministry" desc="Ages 13 – 17" />
    <MobileMinistry href="/ministries#yasm" title="Young Adults & Singles" desc="18+" />
    <MobileMinistry href="/ministries#women" title="Women’s Ministry" desc="" />
    <MobileMinistry href="/ministries#men" title="Men’s Ministry" desc="" />
    <MobileMinistry href="/ministries#wise" title="Wise Ones Ministry" desc="" />
...
// ================== Helper Components ==================
const SocialIcon: React.FC<{ href: string; svgPath: string }> = ({ href, svgPath }) => (
  <a href={href} target="_blank" rel="noopener noreferrer" className="w-8 h-8 bg-red-500 rounded flex items-center justify-center hover:bg-red-600 transition-colors">
...
const MobileItem: React.FC<{ href: string; label: string }> = ({ href, label }) => (
  <Link href={href} className="block bg-gray-700 text-white rounded px-3 py-2 hover:bg-gray-600 transition-colors">
    {label}
  </Link>
)

const MobileMinistry: React.FC<{ href: string; title: string; desc: string }> = ({ href, title, desc }) => (
  <Link href={href} className="block hover:bg-gray-50 transition-colors px-2 py-2 rounded mb-1">
    <div className="text-sm font-semibold text-zinc-800">{title}</div>
    {desc && <div className="text-[11px] text-gray-600">{desc}</div>}
  </Link>
)
