...
interface ActionButtonProps {
  ...
  onClick?: () => void;
  onCloseModal?: () => void; // NEW
}

const ActionButton: React.FC<ActionButtonProps> = ({ children, onClick, onCloseModal }) => {
  const [modalType, setModalType] = React.useState<"signup" | "login" | "success" | null>(null);

  const handleClose = () => {
    setModalType(null);
    onCloseModal?.(); // tell OfferImages to set modalOpen to false
  };
  ...
  return (
    <>
      ...
      {modalType === "signup" && (
        <SignUpModal 
          onClose={handleClose} // still works as before
