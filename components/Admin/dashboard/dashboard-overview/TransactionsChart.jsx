...
function TransactionsChart() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      if (!token) return; // Avoid making API calls without a token
  ...
  return (
    <div className="flex overflow-hidden flex-col flex-1 shrink justify-center p-4 bg-white rounded-lg border border-solid basis-0 border-zinc-300 min-w-[240px] max-md:max-w-full">
      ...
      <div className="flex flex-1 justify-center items-center mt-1 rounded-lg p-4">
        <ReactApexChart options={chartOptions.options} series={chartOptions.series} type="radialBar" height={250} />

        <div className="flex flex-col">
          <div className="flex items-center gap-2 mb-1">
            <div className="w-2 h-2 bg-yellow-500 rounded-full"></div>
            <span className="text-sm text-neutral-500">Pending ({pendingRate}%)</span>
          </div>
          <div className="flex items-center gap-2 mb-1">
            <div className="w-2 h-2 bg-red-600 rounded-full"></div>
            <span className="text-sm text-neutral-500">Failed ({failureRate}%)</span>
          </div>
          <div className="flex items-center gap-2">
            <div className="w-2 h-2 bg-green-600 rounded-full" style={{ backgroundColor: "#08AA3B" }}></div>
            <span className="text-sm text-neutral-500">Success ({successRate}%)</span>
          </div>
        </div>
      </div>
    </div>
