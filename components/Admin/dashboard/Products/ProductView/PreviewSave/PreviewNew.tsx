...
export const PreviewNew = ({ onPrevious, onCancel, uploadedFiles, productId }: PreviewNewProps) => {
  const [description, setDescription] = React.useState("")
  const [effectiveUploadedFiles, setEffectiveUploadedFiles] = React.useState<UploadedFile[]>([])
  ...
  // Handle uploaded files - use new uploads or fall back to existing images from localStorage
  React.useEffect(() => {
    if (uploadedFiles && uploadedFiles.length > 0) {
      // Use newly uploaded files
      setEffectiveUploadedFiles(uploadedFiles)
    } else if (productId) {
      // If editing existing product and no new uploads, try to get existing images from localStorage
      try {
        const existingImages = JSON.parse(localStorage.getItem("uploadedFiles") || "[]")
        if (existingImages && existingImages.length > 0) {
          setEffectiveUploadedFiles(existingImages)
        } else {
          setEffectiveUploadedFiles([])
        }
      } catch (error) {
        console.error("Error parsing existing images from localStorage:", error)
        setEffectiveUploadedFiles([])
      }
    } else {
      // New product with no uploads
      setEffectiveUploadedFiles([])
    }
  ...
  return (
    <main className="overflow-hidden relative p-10 max-w-screen-sm bg-white rounded-2xl max-md:px-5">
      <header className="flex z-0 flex-col justify-center w-full">
        ...
        <h1 className="text-3xl font-semibold text-black leading-[40px]">Preview</h1>
      </header>
        ...
        <article className="mt-6 w-full rounded-lg">
          ...
          <section className="overflow-hidden pt-4 mt-6 w-full border-t border-solid border-t-[color:var(--colour-stroke-default,#D5D5D5)]">
            <div className="flex flex-wrap justify-between items-center w-full">
              <h2 className="text-base font-semibold text-black leading-[20px]">Description</h2>
            </div>
          ...
          <div className="flex gap-2 items-center mt-6 w-full">
            <button className="flex flex-wrap flex-1 shrink gap-2 justify-center items-center self-stretch my-auto w-full h-14 basis-0 bg-[#EAEAEA] min-h-14 min-w-60 rounded-[50px]">
              ...
              <span className="text-base font-medium text-center leading-[24px] text-stone-300">Add to cart</span>
            </button>
          </div>
