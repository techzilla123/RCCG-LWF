...
import type { UploadedFile as ProductDetailFormUploadedFile } from "./ProductDetailForm" // Import the updated interface

// Re-export the updated interface from ProductDetailForm for consistency
export type UploadedFile = ProductDetailFormUploadedFile
...
export const MediaUpload = ({ files, onFilesChange }: MediaUploadProps) => {
  ...
  useEffect(() => {
    ...
    // Ensure 'isExisting' is also saved if present
    const metadata = files.map(({ image, name, type, isExisting }) => ({ image, name, type, isExisting }))
  ...
  const handleFileChange = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    const newFiles: UploadedFile[] = Array.from(selectedFiles).map((file) => ({
      ...
      type: file.type,
      name: file.name,
      file: file, // This will be a real File object
  ...
  return (
    <div className="flex flex-col gap-2">
      ...
      >
        <EyeIcon />
        <span>Drag & drop, or upload at least 3 media files (.jpg, .png, .webp, .mp4, .webm)</span>
        <input
          type="file"
        ...
        {files.map((file, index) => (
          <div key={index} className="flex gap-2 items-center">
            {file.type.startsWith("video") ? (
              <video src={file.image} className="w-10 h-10 rounded-sm bg-stone-50 object-cover" />
            ) : (
              <img
            ...
            <span className="text-xs text-neutral-500 truncate max-w-[100px]">{file.name}</span>
