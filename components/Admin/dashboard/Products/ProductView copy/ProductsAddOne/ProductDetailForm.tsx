...
// Updated UploadedFile interface to include isExisting and make file optional
// This allows distinguishing between newly uploaded files (with a File object)
// and existing files fetched from the API (with just a URL and isExisting flag).
// This interface is also updated in MediaUpload.tsx
// and used in ActionButtons.tsx
// [^1]
export interface UploadedFile {
  image: string // preview URL (can be blob URL for new, or API URL for existing)
  type: string
  name: string
  file?: File // actual File object, present only for new uploads
  isExisting?: boolean // true if fetched from API
}
...
const ProductDetailForm: React.FC<ProductDetailFormProps> = ({ onClose, productId }: ProductDetailFormProps) => {
  ...
  const [uploadedFiles, setUploadedFiles] = useState<UploadedFile[]>([]) // This state holds all files (new and existing)
  ...
  const saveFormDataToLocalStorage = (data: FormData) => {
    ...
    // When saving uploadedFiles to localStorage, only save metadata, not File objects
    const filesMetadata = data.uploadedFiles.map(({ image, name, type, isExisting }) => ({
      image,
      name,
      type,
      isExisting,
    }))
    localStorage.setItem("uploadedFiles", JSON.stringify(filesMetadata))
  }
  ...
  useEffect(() => {
    const fetchProductDetails = async () => {
      ...
      try {
        ...
        // Convert images to UploadedFile format, marking them as existing
        ...
        imageKeys.forEach((key, index) => {
          if (productData[key]) {
            convertedImages.push({
              image: productData[key], // This is the API URL
              name: `Image ${index + 1}`,
              type: "image/jpeg", // Assuming type, or infer from productData if available
              isExisting: true, // Mark as existing
              // No 'file' property for existing images, as we don't have the binary data
            })
          }
        })
        ...
        const newFormData = {
          ...
          url: "", // URL is not part of the fetched product data in the provided code
          ...
          uploadedFiles: convertedImages, // Set uploadedFiles with converted API images
        }
  ...
  const handleNext = () => {
    ...
    // For editing existing products, the `uploadedFiles` state will contain existing images
    // (marked with `isExisting: true`) and any newly uploaded ones.
    // The validation for images will be handled in ActionButtons based on `productId`.
    ...
    Object.entries(formData).forEach(([key, value]) => {
      if (key === "uploadedFiles") {
        // When saving uploadedFiles to localStorage, only save metadata, not File objects
        const filesMetadata = (value as UploadedFile[]).map(({ image, name, type, isExisting }) => ({
          image,
          name,
          type,
          isExisting,
        }))
        localStorage.setItem(key, JSON.stringify(filesMetadata))
