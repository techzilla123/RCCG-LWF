  ...
  const handleAddToCart = async () => {
    ...
    try {
      if (onAddToCart) {
        ...
        const res = await fetch(`${process.env.NEXT_PUBLIC_API_BASE_URL}customer/add-to-cart/${product.id}`, {
          method: "GET",
          headers: {
  ...
  return (
    <article className="relative flex flex-col p-2 bg-white rounded-lg">
      <Link href={`/preview?${product.id}`} className="w-full block">
        <div className="w-full h-[130px] overflow-hidden rounded-t bg-gray-50 flex items-center justify-center">
          <img
      ...
      <div className="mt-2 flex-1">
        ...
        <div className="flex flex-col">
          ...
          <p className="mt-1 font-bold">${Number(product.finalPrice || product.price).toFixed(2)}</p>
        </div>
      </div>
