...
export const MovementSection: React.FC = () => {
  ...
  // Smooth scroll animation for cards only
  React.useEffect(() => {
    let rafId: number;
    ...
    const animate = () => {
      ...
      if (cardsRef.current) {
        cardsRef.current.style.transform = `translate3d(0, ${animatedYRef.current * 0.2}px, 0)`;
  ...
  return (
    <main className="relative bg-white rounded-none">
      <HeroSection />

      <section className="relative flex flex-col items-center px-5 md:px-10 lg:px-20 pb-5 w-full max-w-[1600px] mx-auto bg-white">
        <div className="w-full max-w-[1080px]" ref={cardsRef}>
          {/* Desktop cards */}
          ...
          <div className="md:hidden -mt-12 pb-7">
            <div className="flex gap-4 transition-all duration-500 ease-in-out">
              {[currentIndex, (currentIndex + 1) % movementCards.length].map(
                (index) => (
                  <MovementCard
                    key={index}
